...
const INITIAL_CUSTOMERS = [
  { id: 'CS-001', name: 'Nguyễn Văn A', phone: '0901234567', cccd: '001090123456', email: 'nguyenvana@gmail.com', bookings: 5, created: '10/01/2023', color: 'bg-blue-500' },
  { id: 'CS-002', name: 'Trần Thị B', phone: '0912345678', cccd: '002091234567', email: 'tranthib@gmail.com', bookings: 2, created: '15/03/2023', color: 'bg-purple-500' },
  { id: 'CS-003', name: 'Lê Hoàng C', phone: '0923456789', cccd: '003092345678', email: 'lehoangc@gmail.com', bookings: 1, created: '20/05/2023', color: 'bg-green-500' },
  { id: 'CS-004', name: 'Phạm D', phone: '0934567890', cccd: '004093456789', email: 'phamd@gmail.com', bookings: 8, created: '05/08/2023', color: 'bg-accent-primary' },
];
      ...
      <div className="h-full flex flex-col min-h-0">
        <div className="shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
          <div>
            ...
            <p className="text-text-secondary">Thông tin và lịch sử lưu trú của khách hàng</p>
          </div>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              <tbody className="text-sm">
                {filteredCustomers.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-8 text-center text-text-muted">Không tìm thấy khách hàng nào.</td>
                  </tr>
                ) : (
                  filteredCustomers.map((customer) => (
                    <tr key={customer.id} className="border-b border-border-subtle/50 hover:bg-bg-secondary/50 transition-colors group">
                      <td className="p-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-text-primary font-bold text-lg ${customer.color}`}>
                          {customer.name.charAt(0)}
                        </div>
                      </td>
                      <td className="p-4">
                        <span className="font-mono text-text-secondary">{customer.id}</span>
                      </td>
                      ...
                      <td className="p-4 text-text-secondary font-mono">{customer.cccd}</td>
                      ...
                      <td className="p-4 text-text-muted">{customer.created}</td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button onClick={() => handleDelete(customer.id)} className="p-2 text-text-muted hover:text-danger hover:bg-danger/10 rounded transition-colors" title="Xóa">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="bg-bg-card border border-border-subtle rounded-xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs text-text-muted font-bold mb-1 uppercase tracking-wider">CCCD / Passport *</label>
                <input 
                  type="text" 
                  className="input-field" 
                  required 
