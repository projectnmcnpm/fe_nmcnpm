...
export default function HomePage() {
  ...
  useEffect(() => {
    setFeaturedRooms(mockService.getRooms().slice(0, 9));
  }, []);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="relative h-[85vh] flex items-center justify-center overflow-hidden">
        <div 
          ...
          style={{ backgroundImage: 'url("https://images.unsplash.com/photo-1522771731470-ea43836b6fa0?q=80&w=2070&auto=format&fit=crop")' }}
        />
        <div className="absolute inset-0 z-10 bg-gradient-to-t from-bg-primary via-bg-primary/60 to-transparent" />
        
        <div className="relative z-20 text-center px-4 max-w-5xl mx-auto mt-16">
          <h1 className="text-6xl md:text-8xl lg:text-9xl text-text-primary mb-6 drop-shadow-2xl">PHONG CÁCH SỐNG<br/><span className="text-accent-primary">THẾ HỆ MỚI</span></h1>
          <p className="text-xl md:text-2xl text-text-secondary mb-12 font-light">Homestay GenZ Cinema – Kim Mã | Mỹ Đình | Long Biên</p>
          ...
          <div className="bg-bg-card/80 backdrop-blur-md border border-border-subtle p-4 md:p-6 rounded-2xl shadow-2xl flex flex-col md:flex-row gap-4 items-end">
            <div className="w-full md:w-1/4 text-left">
              <label className="block text-xs text-text-muted font-bold mb-2 uppercase tracking-wider">Cơ sở</label>
            ...
            <div className="w-full md:w-1/4 text-left">
              ...
              <div className="relative">
                ...
                <input type="date" className="input-field bg-bg-secondary pl-10" />
              </div>
            </div>
            <div className="w-full md:w-1/4 text-left">
            ...
            <div className="w-full md:w-1/4">
              <Link href="/rooms" className="btn-primary w-full h-[50px] text-lg flex items-center justify-center">TÌM PHÒNG</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-4 max-w-7xl mx-auto w-full">
        <div className="flex items-end justify-between mb-12">
          <div>
            <h2 className="text-4xl md:text-5xl text-text-primary mb-2">PHÒNG NỔI BẬT</h2>
            <p className="text-text-secondary">Trải nghiệm không gian điện ảnh riêng tư</p>
          </div>
          <Link href="/rooms" className="hidden md:block text-accent-neon hover:text-text-primary transition-colors font-medium">Xem tất cả phòng &rarr;</Link>
        </div>
              ...
              <div className={`p-6 ${room.status === 'full' ? 'opacity-60' : ''}`}>
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="text-right">
                    <div className="text-accent-gold font-display text-2xl">{room.price.toLocaleString('vi-VN')}đ</div>
                ...
                <div className="flex gap-4 mb-6 text-text-secondary">
                  {room.amenities.map((amenity, index) => (
                    <div key={index} className="flex items-center gap-1 text-sm">
                      ...
                      {(amenity.includes('4K') || amenity.includes('HD')) && <MonitorPlay size={16}/>}
        ...
        <div className="mt-12 text-center md:hidden">
          <Link href="/rooms" className="btn-outline inline-block px-8 py-3">Xem tất cả phòng</Link>
        </div>
      </section>
    </div>
  );
